...
The following example showcases a number of features of the Be language.
```
isPrime := (Int p) => Bool {
...
- **List slicing**, similar to how Python does it.
...
- **Semiloaded operators**, one of the core features that are (as far as I know) unique to the Be language. This is described in detail in another section of the specification, but in short it allows you to pass the parameters to an operator one at a time.
  ...
  addThree(5); # 8
  ```
  I'm trying to figure out if I can make this work for any function, but am not sure what the syntax for that should be.
- **Object destructuring** for parameters. A few other objects such as lists and maps also support destructuring:
  ```
  [a, b, ...m, l] = [1...6];
  # a = 1, b = 2, m = [3, 4, 5], l = 6
  ```
- **Null-safety** using the `?` to indicate that a type is nullable (such as `Float?`) and the `??` operator to check whether a variable equals `null`.
